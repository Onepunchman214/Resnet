...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    if Tensorboard:
        ...
        writer.add_scalar('accuracy', total_accuracy/(epoch_size+1), epoch)
    ...
    with tqdm(total=epoch_size_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                with torch.no_grad():
                    val_accuracy = torch.mean((torch.argmax(F.softmax(outputs, dim=-1), dim=-1) == targets).type(torch.FloatTensor))
    ...
    if Tensorboard:
        ...
        writer.add_scalar('val_accuracy',val_total_accuracy / (epoch_size_val+1), epoch)        
...
if __name__ == "__main__":
    ...
    #------------------------------------------#
    # 加快模型训练的效率
    model_path = "model_data/resnet50.pth"
